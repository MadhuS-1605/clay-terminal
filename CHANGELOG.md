...
- Added a Kitty color scheme (`terminal/kitty.conf`), generated by a new `icons/generate_kitty.py` directly from the dark theme's own `terminal.ansi*` colors — no duplicated hex values. Downloadable, not bundled in the `.vsix` — fourth and final format shipped under ROADMAP.md's "Terminal emulator color schemes" item, after Windows Terminal in 0.7.6, iTerm2 in 0.7.7, and Alacritty in 0.7.8. That roadmap item is now fully shipped.

## 0.7.8

- Added an Alacritty color scheme (`terminal/alacritty.toml`), generated by a new `icons/generate_alacritty.py` directly from the dark theme's own `terminal.ansi*` colors — no duplicated hex values. Downloadable, not bundled in the `.vsix` — third format shipped under ROADMAP.md's "Terminal emulator color schemes" item, after Windows Terminal in 0.7.6 and iTerm2 in 0.7.7; Kitty remains.

## 0.7.7

- Added an iTerm2 color preset (`terminal/clay-terminal.itermcolors`), generated by a new `icons/generate_itermcolors.py` directly from the dark theme's own `terminal.ansi*` colors — no duplicated hex values, written as a valid property list via Python's `plistlib`. Downloadable, not bundled in the `.vsix` — second format shipped under ROADMAP.md's "Terminal emulator color schemes" item, after Windows Terminal in 0.7.6; Alacritty/Kitty remain.

## 0.7.6

- Added a Windows Terminal color scheme (`terminal/windows-terminal.json`), generated by a new `icons/generate_windows_terminal.py` directly from the dark theme's own `terminal.ansi*` colors — no duplicated hex values, unlike the Starship/Oh My Posh generators which import from `icons/palette.py`. Downloadable, not bundled in the `.vsix` — first format shipped under ROADMAP.md's "Terminal emulator color schemes" item; iTerm2 and Alacritty/Kitty remain.
