...
Current state (v0.7.8): 4 color theme variants (dark, light, hc-black,
...
(`terminal/ohmyposh.json`), a Windows Terminal color scheme
(`terminal/windows-terminal.json`), an iTerm2 color preset
(`terminal/clay-terminal.itermcolors`), and an Alacritty color scheme
(`terminal/alacritty.toml`), all downloadable and not bundled in
...
- **Terminal emulator color schemes (remaining formats)** — Kitty conf,
  following the same pattern as `icons/generate_windows_terminal.py`,
  `icons/generate_itermcolors.py`, and `icons/generate_alacritty.py` (reads
  the 16 ANSI colors straight from `themes/claude-code-color-theme.json`
  rather than duplicating hex values). Windows Terminal shipped in v0.7.6,
  iTerm2 in v0.7.7, Alacritty in v0.7.8; this one remains. Ships as a
  downloadable asset linked from the README, not bundled in the extension
  package.
- **Powerlevel10k prompt preset** — same idea as the Starship and Oh My Posh
  presets, for users on that prompt framework. Higher effort than the other
  two: p10k configs are generated zsh scripts (`~/.p10k.zsh`), not a single
  declarative TOML/JSON file, so this needs its own generator shape rather
  than reusing the Starship/Oh My Posh pattern directly. Same distribution
  model (downloadable asset, not bundled).
