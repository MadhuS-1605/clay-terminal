...
Current state (v0.7.3): 4 color theme variants (dark, light, hc-black,
...
icon generator, all 4 color themes, and a Starship prompt preset
(`terminal/starship.toml`, downloadable, not bundled in the extension),
and CI validation (`icons/validate_theme.py`) covering both icon themes'
...
- **Oh My Posh / Powerlevel10k prompt presets** — same idea as the Starship
  preset shipped in v0.7.3, for users on a different prompt framework. Same
  distribution model (downloadable asset, not bundled).
