...
Current state (v0.7.7): 4 color theme variants (dark, light, hc-black,
...
(`terminal/windows-terminal.json`), and an iTerm2 color preset
(`terminal/clay-terminal.itermcolors`), all downloadable and not bundled in
...
- **Terminal emulator color schemes (remaining formats)** — Alacritty/Kitty
  TOML/conf, following the same pattern as `icons/generate_windows_terminal.py`
  and `icons/generate_itermcolors.py` (reads the 16 ANSI colors straight
  from `themes/claude-code-color-theme.json` rather than duplicating hex
  values). Windows Terminal shipped in v0.7.6, iTerm2 in v0.7.7; this one
  remains. Ships as a downloadable asset linked from the README, not
