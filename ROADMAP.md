...
Current state (v0.7.6): 4 color theme variants (dark, light, hc-black,
...
(`terminal/ohmyposh.json`), and a Windows Terminal color scheme
(`terminal/windows-terminal.json`), all downloadable and not bundled in the
extension, and CI validation (`icons/validate_theme.py`) covering both icon themes'
...
- **Terminal emulator color schemes (remaining formats)** — `.itermcolors`
  (iTerm2) and Alacritty/Kitty TOML/conf, following the same pattern as
  `icons/generate_windows_terminal.py` (reads the 16 ANSI colors straight
  from `themes/claude-code-color-theme.json` rather than duplicating hex
  values). Windows Terminal shipped in v0.7.6; these two remain. Ships as
  downloadable assets linked from the README, not bundled in the extension
  package.
